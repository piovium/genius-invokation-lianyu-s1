...
import { formatPercent, formatQuantity } from "./StatisticsValue";

export interface StatisticsActionCardProps {
  id: number;
  appearances: number;
...
export function StatisticsActionCard(props: StatisticsActionCardProps) {
  ...
  const items = () => [
    ...
    { label: "平均携带", value: formatQuantity(props.averageCopies) },
    { label: "净携带", value: formatQuantity(props.netCopies) },
  ];

  return (
    <article class="min-h-28 flex items-center gap-4 rounded-lg b b-gray-2 bg-white p-3">
      <img
        class="h-22 w-22 shrink-0 rounded-lg bg-gray-1 object-cover object-[center_35%]"
        src={assetsManager().getImageUrlSync(props.id, { type: "cardFace" })}
        alt={name()}
      />
